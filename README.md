...
For this particular execution strategy, Sub-Orchestrator were used to perform the activities. Due to the limitations of api call rates, fan-in and fan-out is not feasible. So therefore, all the activities were enclosed solely within a sub-orchestrators respectively. This is done to achieve parallel execution on the orchestrating level. This was accomplished by creating a list of sub-orchestrators names and creating a task all to provision tasks (call all the sub-orchestrators at once). By setting this configuration, we can run multiple device provisioning flows in parallel to facilitate the execution strategy by invoking to the parent orchestrator.
